...
function layout(title: string, active: string, body: string) {
  const nav = [
    { href: "/", label: "Overview" },
    { href: "/process-lane", label: "Process Lane" },
    { href: "/governance-gaps", label: "Governance Gaps" },
    { href: "/execution-posture", label: "Execution Posture" },
...
<html lang="en">
  ...
  <body>
    ...
    <div class="wrap">
      <div class="topbar">
        <div class="left">Kinetic Gain · Camunda Process Governance</div>
        <div class="right">
          <div>synthetic process packets · checkpoint review</div>
          <div>workflow · enterprise integration · bpm operations</div>
        </div>
      </div>
      <div class="herorow">
        <section class="hero">
          <div class="chiprow">
            ...
            <span class="meta-chip">Camunda process-governance proof</span>
            <span class="meta-chip">Synthetic process + exception exports</span>
          </div>
          <h1>Process packets that stay operator-readable.</h1>
          <p>This control plane turns Camunda process and task data into one buyer-readable surface: missing approval packets, routing attestation drift, stale SLA timers, compensating-action gaps, and the exact sequence needed before a checkpoint, audit, or incident review hardens.</p>
          <div class="navrow">
        ...
        <aside class="side">
          <div class="bluf">
            ...
            <p><strong>Workflow governance evidence for BPM owners, release teams, and process auditors.</strong><br />Audit-safe visibility into missing approval packets, timer drift, handoff evidence, and compensating-action posture without exposing live workflow payloads.</p>
          </div>
          <div class="corr">
            <div class="lbl">Proof Layer</div>
            <p><strong>Offline analyzer + CLI + dashboard surface.</strong><br />This repo reads process and exception snapshots and turns them into lane, gap, and execution packets operators can actually use.</p>
          </div>
          <div class="corr">
            <div class="lbl">Why it matters</div>
            <p>Recruiters looking for <strong>Camunda / BPMN / workflow governance / process routing / release operations</strong> should see real operator work: approval evidence, task handoffs, timer posture, and owner-safe escalation.</p>
          </div>
        </aside>
      </div>
      ${body}
      <div class="footer">
        <div>camunda-process-governance · synthetic sample data only</div>
        <div>routes: / · /process-lane · /governance-gaps · /execution-posture · /verification · /docs</div>
      </div>
    </div>
  </body>
</html>`;
...
export function renderOverview() {
  ...
  return layout(
    ...
    `<section class="section">
        <div class="sh"><h2>Operator Snapshot</h2><div class="note">checkpoint pressure · routing health · execution posture</div></div>
        <div class="kpis">
          <div class="kpi cyan"><div class="v">${metrics.processes}</div><div class="lbl">processes</div><div class="h">Synthetic process runs modeled through task, timer, and checkpoint pressure.</div></div>
          <div class="kpi green"><div class="v">${metrics.onTrackProcesses}</div><div class="lbl">on-track processes</div><div class="h">Runs currently carrying healthy packet and routing posture.</div></div>
          <div class="kpi plum"><div class="v">${metrics.packets}</div><div class="lbl">packets</div><div class="h">Governance packets across approval, routing, timer, and escalation domains.</div></div>
          ...
          <div class="kpi amber"><div class="v">${metrics.workflowGaps}</div><div class="lbl">workflow gaps</div><div class="h">Process routes still missing healthy owner or escalation sequencing.</div></div>
          <div class="kpi red"><div class="v">${metrics.stalePackets}</div><div class="lbl">stale packets</div><div class="h">Open evidence packets older than the workflow review SLA.</div></div>
        </div>
      </section>
      <section class="section">
        <div class="sh"><h2>Why operators care</h2><div class="note">release trust · packet routing · audit prevention</div></div>
        <div class="stack">
          ...
          <div class="src"><div class="src-name">operator evidence</div><div class="src-tit">Turn process exports into checkpoint proof</div><p>Every lane stays tied to owner, process, packet readiness, and the next concrete operator move.</p></div>
          <div class="src"><div class="src-name">recruiter signal</div><div class="src-tit">Show real BPM governance depth</div><p>This is real process-governance routing and execution posture proof, not generic workflow copy.</p></div>
        </div>
      </section>`
  );
}

export function renderProcessLane() {
  return layout(
    "Camunda Process Governance — Process Lane",
    "/process-lane",
    `<section class="section">
        <div class="sh"><h2>Process Lane</h2><div class="note">owner · focus · next action</div></div>
        <table class="ttbl">
          <thead>
            <tr>
              <th>Lane</th>
...
export function renderGovernanceGaps() {
  return layout(
    "Camunda Process Governance — Governance Gaps",
    "/governance-gaps",
    `<section class="section">
        <div class="sh"><h2>Governance Gaps</h2><div class="note">severity · owner · packet</div></div>
        <table class="ttbl">
          <thead>
            <tr>
              <th>Gap</th>
...
export function renderExecutionPosture() {
  return layout(
    "Camunda Process Governance — Execution Posture",
    "/execution-posture",
    `<section class="section">
        <div class="sh"><h2>Execution Posture</h2><div class="note">packet readiness · blocker · checkpoint window</div></div>
        <div class="board">
          ${executionPosture().map((packet) => `<article class="pcard">
              <div class="ptop">
...
export function renderValidation() {
  return layout(
    "Camunda Process Governance — Verification",
    "/verification",
    `<section class="section">
        <div class="sh"><h2>Verification</h2><div class="note">operator-safe claims only</div></div>
        <div class="stack">
          ${verification().map((item, index) => `<div class="src"><div class="src-name">verification ${index + 1}</div><div class="src-tit">${item}</div><p>This surface stays explicit about offline exports, synthetic sample data, and real process-governance posture.</p></div>`).join("")}
        </div>
      </section>`
  );
}
...
export function renderDocs() {
  return layout(
    "Camunda Process Governance — Docs",
    "/docs",
    `<section class="section">
        <div class="sh"><h2>Docs</h2><div class="note">routes · cli · api</div></div>
        <div class="stack">
          <div class="src"><div class="src-name">routes</div><div class="src-tit">Public control surface</div><p><code>/</code>, <code>/process-lane</code>, <code>/governance-gaps</code>, <code>/execution-posture</code>, <code>/verification</code>, <code>/docs</code></p></div>
          <div class="src"><div class="src-name">api</div><div class="src-tit">Structured payloads</div><p><code>/api/dashboard/summary</code>, <code>/api/process-lane</code>, <code>/api/governance-gaps</code>, <code>/api/execution-posture</code>, <code>/api/verification</code>, <code>/api/sample</code></p></div>
          <div class="src"><div class="src-name">cli</div><div class="src-tit">Offline governance analysis</div><p><code>npx camunda-process-gov fixtures/camunda-processes-clean.json --format summary</code> renders the same checkpoint posture the dashboard exposes.</p></div>
        </div>
      </section>`
  );
}
