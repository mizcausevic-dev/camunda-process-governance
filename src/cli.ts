...
async function main() {
  ...
  const input = args[0];
  if (!input) {
    console.error("Usage: camunda-process-gov <export.json> [--format markdown|summary] [--stale-hours N]");
    process.exitCode = 1;
    return;
  }
  ...
  const payload = JSON.parse(await readFile(input, "utf8")) as CamundaGovernanceExport;
