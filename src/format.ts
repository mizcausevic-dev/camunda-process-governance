...
export function toSummary(report: CoverageReport) {
  ...
  return `${posture} ${report.processes} processes · ${report.packets} packets · ${report.highSeverityPackets} high-severity packets · ${report.workflowGaps} workflow gaps`;
}
...
export function toMarkdown(report: CoverageReport) {
  ...
  return [
    `# Process governance posture ${report.ok ? "✅" : "❌"}`,
    "",
    `- Processes: **${report.processes}**`,
    `- On-track processes: **${report.onTrackProcesses}**`,
